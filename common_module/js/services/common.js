		...
		config(function ($compileProvider,$provide) {
			$compileProvider.
					...
					compile:function(elem, attrs, childTranscludeFn){
						return function(scope,elem,attrs,ngModelController){
							scope.$watch('regExp',function(newVal){
								if(newVal && (newVal.indexOf( 'common.require')>-1 ||newVal.indexOf( 'FormValidate.isNotNull')>-1)){
									scope._regExp = true;
								}else {
									scope._regExp = false;
								}
				...
				return {
					...
					'<div name="leftP" style="position: absolute;top: -webkit-calc(50% - 10px);top: calc(50% - 10px);">'+
					'<span style="text-align:left, font-size: 12px">'+
					'<b ng-if="_regExp" style="color: red;">*</b>{{name}}</span></div>' +
					'<div name="rightP"><textarea class="input"  ng-style="textarea" id="{{htId}}" ng-disabled="disable&&disable!==\'false\'"' +
					...
					compile:function(elem, attrs, childTranscludeFn){
						return function(scope,elem,attrs,ngModelController){
							...
							var resize = function(){
								...
								var curentDom = angular.element(elem);
								curentDom.find('div[name=leftP]').css({
									width: labelWidth, 
									'padding-left': labelPadding
								});
								curentDom.find('div[name=rightP]').css('margin-left',labelWidth);
							}
							resize();
				...
				return {
					...
					'<div ng-if="extentDiv" style="max-height:300px;" ng-switch="extent.type" class="inputContent">' +
				...
				return {
					...
					template:
					'<div ng-style="label" ng-show="!hide || hide==\'false\'"><div ng-style="radio" class="radio">' +
					...
					'ng-change="changeradio(option.dataVal)"'+
					'ng-model="radio.tempModel" value="{{option.dataVal}}" ng-disabled="disable&&disable!==\'false\'">' +
					'<label style="width: 100%; font-size: 12px;white-space:nowrap;padding-top:2px;" for="{{$index+htId}}" ng-bind="option.dataTrsVal"></label>' +
					'</li></ul></div>' +
					'</div></div>',
					compile:function(elem, attrs){
						return function(scope,elem,attrs,ngModelController){
							...
							scope.label = {width:"100%",
								color:"#393939",fontWeight: "normal", marginBottom: "5px"};
							scope.radio = {
				...
				return {
					...
					template:
					'<div ng-style="label" ng-show="!hide || hide==\'false\'"><div ng-style="checkbox" class="checkbox">' +
					'<div name="leftP" style="top: -webkit-calc(50% - 10px);top: calc(50% - 10px);position: absolute;">' +
					'<span ng-style="span" ng-bind="name"></span></div>' +
					'<div name="rightP"><ul style="display: inline-block;margin: 0" ht-dictionary="{{dictionary}}">' +
					'<li style="display: inline-block;padding-right: 10px" ng-repeat="option in dicArr">'+
					...
					'<label style="width: 100%; font-size: 12px;white-space:nowrap;padding-top:1px;" for="{{$index+htId}}" ng-bind="option.dataTrsVal"></label>' +
					'</li></ul></div>' +
					'</div></div>',
					compile:function(elem, attrs){
						return function(scope,elem,attrs,ngModelController){
				...
				return{
					...
					'<input class="input" id="{{htId}}" type="text" ng-model="s.select" ng-disabled="disable&&disable!==\'false\'"' +
					...
					link:function(scope,elem,attrs,ngModelController){
						...
						scope.changeOption = function(option){
							...
							if(option.dataVal == scope.modelVal) return;
				...
				return{
					...
					'<div ui-view></div>' +
					'<div class="button">' +
					'<button class="btn btn-primary" ng-click="pre()" ng-disabled="disablePre">上一步</button>&nbsp;&nbsp;' +
					'&nbsp;&nbsp;<button class="btn btn-primary" ng-click="next()" ng-disabled="disableNext">下一步</button></div>' +
					'</div>',
					link:function(scope,elem,attrs){
						scope.ul.liWidth = {
					...
					link :function($scope,element,attrs){
						...
						if($scope.type=="download"){
							...
							div.on('click',function (){
								...
								// sendUrl = 'http://localhost:8000/scubeUI1.1/scube_ui/config.txt';
								form.prop({
									action: sendUrl,
									method: 'get',
									target: iframe.prop('name'),
								...
								iframe[0].onload = function() {
									var doc = document.frames ? document.frames[iframe.prop('name')].document : iframe[0].contentDocument;
            						var contentWindow = document.frames ? document.frames[iframe.prop('name')].window : iframe[0].contentWindow;
									var data;
									var resData = doc.body.getElementsByTagName('pre')[0].innerHTML;
									// resData = '{"scubeBody":{"contextData":{"data":"%5B%22%5C%5Capps%5C%5Cscube%5C%5C20170118%5C%5C%E5%8A%9F%E8%83%BD%E9%9C%80%E6%B1%82.txt%22%2C%22%5C%5Capps%5C%5Cscube%5C%5C20170118%5C%5C%E8%B4%AD%E6%88%BF%E8%B4%B7%E6%AC%BE%E4%BA%86%E5%85%B3.png%22%2C%22%5C%5Capps%5C%5Cscube%5C%5C20170118%5C%5C%E8%B7%AF%E7%94%B1%E4%BC%A0%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE.docx%22%2C%22%5C%5Capps%5C%5Cscube%5C%5C20170118%5C%5C%E7%BB%9F%E4%B8%80%E5%AE%A2%E6%88%B7%E7%AB%AF-%E5%B7%A5%E4%BD%9C%E5%AE%89%E6%8E%92.xlsx%22%5D","domainVilidate":"domainVilidate"}},"scubeHeader":{"errorCode":"SUC","errorMsg":"","transCode":"","x-session-token":"x-session-token"}}';
									...
									resData = {
										...
										errorMsg: resData.scubeHeader.errorMsg
									};
									if($scope.option.callbackFun){
										$scope.option.callbackFun(resData);
									}
									form.remove();
								};
								form.submit();
							...
							var changeed = function(){
								...
								$scope.$apply(function(){
										...
										if(window.ActiveXObject){
											...
											var iframe = angular.element('<iframe name="iframeTransport' + new Date().valueOf() + '" style="display: none;">');
											//var authorization ={name:'Authorization',value: window.authorization||sessionStorage.getItem("authorization")};
					...
					'responseError' : function(response) {
						if (response.status == 401) {
							...
							if(response.data.redirectLoginUrl && response.data.redirectLoginUrl.indexOf("http")!=-1){
								...
								_href.pop();
								_href.pop();
								window.top.location.href = _href.join('/');
							}
			...
			$provide.factory('$common',function ($http,$q,ngDialog) {
				var common = {
					...
					get_asyncData:function(url,data,ajaxfun,hraders){
						...
						ajaxfun.type = (ajaxfun.type == null || ajaxfun.type == "" || typeof (ajaxfun.type) == "undefined") ? _type : ajaxfun.type;
