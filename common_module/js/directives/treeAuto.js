    ...
    directive('htAutotree', function($common,$compile){
        var linkFunc = function (scope, element, attrs) {
            ...
            var buildAllChildNode = function(fatherId ,nodes) {
                // STEP-1 写树下所有节点的html
                if(null != nodes) {
                    nodes = transformTozTreeFormat(nodes);
                    var tempHtml = appendNodes(nodes);
                    if(tempHtml != ''){
                        ...
                        //         // var el1 = element.find('div[tR=tD' +everyChild['treeParentId']+ ']');
                        //         // var el2 = element.find('div[ftR=' +everyChild['treeParentId']+ ']');
                        //         // el2.appendTo(el1);	// node节点按层级移位   
                        //     }
                        // }
                    }
                }
            }
            var appendNodes = function(nodes){
                var everyChild,tempHtml = '';
                for (var i = 0 ; i < nodes.length ; i++ ) {
                        everyChild = nodes[i];
                        ...
                        tempHtml += '</span></div><div tR="tD' +everyChild['treeId']+ '" ftR="' +everyChild['treeParentId']+ '" class="pl10" ng-style="tDStyle">';
                        if (everyChild['child'] && everyChild['child'].length > 0) {
                            tempHtml += appendNodes(everyChild['child']);
                        }
                        tempHtml +='</div>';
                }
                return tempHtml;
            ...
            var transformTozTreeFormat = function(sNodes) {
                ...
                if (Array.isArray(sNodes)) {
                    ...
                    for (i=0, l=sNodes.length; i<l; i++) {
                        tmpMap[sNodes[i]['treeId']] = sNodes[i];
                    }
                    for (i=0, l=sNodes.length; i<l; i++) {
                        if (tmpMap[sNodes[i]['treeParentId']] && sNodes[i]['treeId'] != sNodes[i]['treeParentId']) {
                            if (!tmpMap[sNodes[i]['treeParentId']]['child'])
                                tmpMap[sNodes[i]['treeParentId']]['child'] = [];
                            tmpMap[sNodes[i]['treeParentId']]['child'].push(sNodes[i]);
                        } else {
                            r.push(sNodes[i]);
                        }
                    }
