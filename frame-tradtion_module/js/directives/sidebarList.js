    ...
    directive('sidebarUl',function ($interval,$window,$common,$compile) {
        return {
            ...
            link : function (scope,elem,attrs) {
                ...
                var clickToPage2 =function (event) {
                    var target = window.event.target || window.event.srcElement;
                    if(target.tagName.toLowerCase() !== 'a') {
                        target = target.parentNode;
                    }
                    var item = target.getAttribute('menuitem');
                    toSysPage(JSON.parse(item));
                };
                ...
                scope.creatSidebar = function(firstLevel){            
                    if(angular.isArray(scope.sidebarList)){
                        var c_temp;//临时模板对象
                        var sidebarTemp = {};//模板对象
                        var tempDiv = angular.element(document.body).find("#sidebarList").empty();
                        ...
                        angular.forEach(_sidebarObj,function(value,index){
                            ...
                            if(index == '0' || (firstLevel != undefined && index ==firstLevel)){
                                var html = '';
                                var i_class = 'position:absolute;left:1px';
                                var span_class = 'margin-left: 30px;display: inline-block;line-height:1.5;padding-top:14px;padding-right: 9px;';
                                var span_class2 = 'margin-left: 5px;display: inline-block;line-height:1.5;padding-top14px;padding-right: 9px;';
                                for(var i=0;i<value.length;i++){
                                    html +='<li menucode="'+value[i].menuCode+'">' +
                                    "<a href='#' menuitem='"+JSON.stringify(value[i])+"'>";
                                    if(value[i].iconName){
                                        html +='<i class="'+value[i].iconName+'" style="'+i_class+'"></i>'+
                                        '<span class="menu-text" style="'+span_class+'">'+value[i].menuName+'</span></a></li>';
                                    }else{
                                        html +='<i></i>'+
                                        '<span class="menu-text" style="'+span_class2+'">'+value[i].menuName+'</span></a></li>';
                                    }
                                }
                            }else {
                        ...
                        do {
                            ...
                            angular.forEach(sidebarTemp,function(value,index){
                                ...
                                var liDOM = tempDiv.find("li[menucode="+index+"]");
                                if(liDOM.length == 1){
                                    liDOM.append(value);
                                    var liA = tempDiv.find("li[menucode="+index+"]>a");
                                    liA.addClass('dropdown-toggle')
                                    liA.append('<b class="arrow icon-angle-right"></b>');
                                    delete sidebarTemp[index];
                                }
                            })
                        }while (Object.keys(sidebarTemp).length>0 && !angular.equals(sidebarTemp,c_temp));
                        tempDiv.find('a').on('click', clickToPage2);
                    }
                };
            },
            replace :true
